...
from efficienedit.utils import norm_logits, sample, Timer
...
class KVCacheModel():
    ...
    def _forward_with_kvcache(self, input_ids : torch.Tensor, prompt_len, use_debug = True) -> torch.Tensor:
        if self._past_key_values is None:
            ...
            self.timer.start("prefill")
            self.timer.start("prefill_forward")
            outputs = self._model(input_ids)
            self.timer.stop("prefill_forward")
            ...
            self.timer.stop("prefill", len(input_ids.flatten()))
        else:
            ...
            # print(last_input_id)
            self.timer.start("decode_forward")
            outputs = self._model(last_input_id, past_key_values=self._past_key_values, use_cache=True)
            self.timer.stop("decode_forward")
            ...
            self._past_key_values = outputs.past_key_values
            self.timer.stop("decode", len(last_input_id.flatten()))
